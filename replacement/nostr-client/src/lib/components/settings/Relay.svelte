<script lang="ts">
	// ABOUTME: Relay settings — add, remove, and list Nostr relays with read/write/search flags.
	// ABOUTME: Fetches relay list on mount and updates it via the backend API.
		...
		fetch(`${import.meta.env.VITE_API_LINK}/api/addrelay`, {
			...
			.then((response) => {
				...
				if (response.status == 'ok') {
					...
					url = ''
					relays = response.data ? response.data : []
				}
	...
	<div class="block p-6 rounded-lg shadow-lg w-full ml-6 mt-6 bg-blue-200">
		<form onsubmit={(e) => e.preventDefault()}>
			<div class="row">
				...
				<div class="flex justify-end w-full gap-2">
					<div class="col-2">
						<button type="button" onclick={addRelay} class="btn">
				...
				{#each relays as relay (relay.url)}
					<div class="flex justify-between space-x-1 p-2">
						...
						<div class="flex justify-end">
							<button type="button" onclick={() => removeRelay(relay.url)} class="btn-remove">
