...
const imgUrlRegex = /https?:\/\/.*\.(png|jpe?g|gif|webp)/gim;
const youtubeRegex =
...
export function toHtml(content: string): string {
  ...
  if (match && match[0]) {
    ...
    for (let i = 0; i < invoice.sections.length; i++) {
      const section = invoice.sections[i] as any;
      if (section?.name == "amount") {
        rawAmount = section.value;
        rawUnit = section.letters;
        amount = section.value;
        const unit = section.letters.replace(/[0-9]+/, "");
        unitNumber = section.letters.replace(/[a-zA-Z]+/, "");
        switch (unit) {
          case "m":
